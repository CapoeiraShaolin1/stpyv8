...
class Navigator(PyV8.JSClass):
    ...
    @property
    def urlheaders(self):
        hd = {  'User-Agent': self.userAgent,
                'Referer': self._win.url}
        if self._win.doc.cookie:
            hd['Cookie'] = self._win.doc.cookie
        return 
    ...
    def fetch(self, url):
        ...
        # req.add_header('Referer', self._win.url)
        # if self._win.doc.cookie:
        #     req.add_header('Cookie', self._win.doc.cookie)

        # response = urlopen(req)


        http = PoolManager()
        response = http.request('GET', url, headers=self.urlheaders)
        ...
        if response.status != 200:            
            ...
            raise HTTPError(url, response.status,  http_status_description, 0)
...
class HtmlWindowTest(unittest.TestCase):
    ...
    def testWindow(self):
        self.assertEqual(self.doc, self.win.document)
        self.assertEqual(self.win, self.win.window)
        self.assertEqual(self.win, self.win.self)

        self.assertFalse(self.win.closed)
        self.win.close()
        self.assertTrue(self.win.closed)

        self.assertEqual(2, self.win.frames.length)
        self.assertEqual(2, self.win.length)
        
        self.assertEqual(1, self.win.history.length)

        loc = self.win.location

        self.assertTrue(loc)
        self.assertEqual("frag", loc.hash)
        self.assertEqual("localhost:8080", loc.host)
        self.assertEqual("localhost", loc.hostname)
        self.assertEqual(TEST_URL, loc.href)
        self.assertEqual("/path", loc.pathname)
        self.assertEqual(8080, loc.port)
        self.assertEqual("http", loc.protocol)
        self.assertEqual("query=key", loc.search)

    def testOpen(self):
        url = 'http://www.google.com'
        win = self.win.open(url, specs="width=640, height=480")
        self.assertEqual(url, win.url)

        self.assertTrue(win.document)
        self.assertEqual(url, win.document.URL)
        self.assertEqual('www.google.com', win.document.domain)
        self.assertEqual(640, win.innerWidth)
        self.assertEqual(480, win.innerHeight)
    ...
    def testScript(self):
        ...
        self.assertEqual('P', tag.nodeName)
