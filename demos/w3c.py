...
class NamedNodeMap(PyV8.JSClass):
    ...
    def item(self, index):
        ...
        names = list(dict(self.parent.tag.attrs).keys())    # TODO
        return self.parent.getAttributeNode(names[index]) if 0 <= index and index < len(names) else None
    
    @property
    def length(self):
        return len(dict(self.parent.tag.attrs))
        # return len(self.parent.tag._getAttrMap())    # TODO _getAttrMap() is depricated, so, dict(doc.tag.attrs)
...
class Attr(Node):
    ...
    def specified(self):
        return self.attr in str(self.parent)
    
    def getValue(self):
        if self.parent:
            # if self.attr in str(self.parent.tag):    # TODO
            if self.attr in list(dict(self.parent.tag.attrs).keys()):
...
class Element(Node):
    ...
    def getAttribute(self, name):
        return self.tag[name] if name in str(self.tag) else ""
    ...
    def getAttributeNode(self, name):
        return Attr(self, name) if name in str(self.tag) else None
...
class Document(Node):
    ...
    @property
    def doctype(self):
        for tag in self.doc.find_all():   # TODO
            if isinstance(tag, BeautifulSoup.Declaration) and tag.startswith("DOCTYPE"):
                return DocumentType(self.doc, tag)  
        return None
    """
    # https://stackoverflow.com/questions/2499358/get-document-doctype-with-beautifulsoup
    @property    
    def doctype(self):   # TODO
        items = [item for item in self.doc.contents if isinstance(item, BeautifulSoup.Doctype)]
        if items:
            return DocumentType(self.doc, items[0])
        return None
    ...
    def createElement(self, tagname):
        ...
        element = DOMImplementation.createHTMLElement(self, self.doc.new_tag(tagname))
...
def attr_property(name, attrtype=str, readonly=False, default=None):
    def getter(self):     
        return attrtype(self.tag[name]) if name in str(self.tag) else default
...
class CSSStyleDeclaration(object):
    ...
    def item(self, index):
        ...
        return self.props[list(self.props.keys())[index]]   # PY3
...
class ElementCSSInlineStyle(object):
    @property
    def style(self):
        return CSSStyleDeclaration(self.tag['style'] if 'style' in str(self.tag) else '')
...
class HTMLElement(Element, ElementCSSInlineStyle):    
    ...
    def innerHTML(self, html):
        dom = BeautifulSoup.BeautifulSoup(html, features="lxml")
...
def xpath_property(xpath, readonly=False):
    ...
    def getChildren(tag, parts, recursive=False):
        ...
        if idx:
            if idx[0] == '@':
                tags = [tag for tag in tags if idx[1:] in str(tag)]  #  TODO
            else:
                tags = [tags[int(idx)-1]]
        ...
        children = [i for i in children if i]    # remove None from list
...
class HTMLDocument(Document):
    ...
    @property
    def domain(self):
        return parse_url(self._win.url).hostname if self._win else ''
    ...
    def close(self):
        ...
        self.doc = BeautifulSoup.BeautifulSoup(html, features="lxml")
    ...
    def write(self, html):
        if self._html:
            ...
            #  for testWrite()
            #    features="lxml"         returns <html><head><meta/></head></html>
            #    features="html.parser"  returns <meta/>
            #
            for tag in BeautifulSoup.BeautifulSoup(html, features="html.parser").contents:   # TODO
...
class DOMImplementation(HTMLDocument):
    ...
    def createHTMLElement(doc, tag):
        # check = True if tag.name.lower() in DOMImplementation.TAGS else None
...
def parseString(html, **kwds):
    return DOMImplementation(BeautifulSoup.BeautifulSoup(html, features="lxml"), **kwds)
...
class DocumentTest(unittest.TestCase):
    ...
    def testNode(self):
        self.assertEqual(Node.DOCUMENT_NODE, self.doc.nodeType)
        self.assertEqual("#document", self.doc.nodeName)
        self.assertFalse(self.doc.nodeValue)
        
        html = self.doc.documentElement
        
        self.assertTrue(html)        
        self.assertEqual(Node.ELEMENT_NODE, html.nodeType)
        self.assertEqual("HTML", html.nodeName)
        self.assertFalse(html.nodeValue)
        
        attr = html.getAttributeNode("xmlns")
        
        self.assertTrue(attr)

        self.assertEqual(Node.ATTRIBUTE_NODE, attr.nodeType)
        self.assertEqual("xmlns", attr.nodeName)
        self.assertEqual("http://www.w3.org/1999/xhtml", attr.nodeValue)
        
    def testNodeList(self):
        nodes = self.doc.getElementsByTagName("body")
        
        self.assertEqual(1, nodes.length)
        
        self.assertTrue(nodes.item(0))
        self.assertFalse(nodes.item(-1))
        self.assertFalse(nodes.item(1))

        self.assertEqual(1, len(nodes))

        self.assertTrue(nodes[0])
        self.assertFalse(nodes[-1])
        self.assertFalse(nodes[1])
    ...
    def testDocument(self):
        ...
        self.assertEqual("BODY", body.tagName)   
    
    def testDocumentType(self):
        doctype = self.doc.doctype
        self.assertTrue(doctype)
        
        self.assertEqual("html", doctype.name)
                
    def testElement(self):
        html = self.doc.documentElement
        
        self.assertEqual("HTML", html.tagName)
        self.assertEqual("http://www.w3.org/1999/xhtml", html.getAttribute("xmlns"))
        self.assertTrue(html.getAttributeNode("xmlns"))
        ...
        self.assertTrue(div)
        self.assertFalse(div.hasChildNodes())
        self.assertEqual(0, len(div.childNodes))
        
        a = self.doc.createElement("a")
        b = self.doc.createElement("b")
        p = self.doc.createElement("p")
        
        self.assertTrue(a == div.appendChild(a))
        self.assertTrue(div.hasChildNodes())
        self.assertEqual(1, len(div.childNodes))        
        self.assertTrue(a == div.childNodes[0])
        
        self.assertTrue(b == div.insertBefore(b, a))
        self.assertEqual(2, len(div.childNodes))
        self.assertTrue(b == div.childNodes[0])
        self.assertTrue(a == div.childNodes[1])
        
        self.assertTrue(a == div.replaceChild(p, a))
        self.assertEqual(2, len(div.childNodes))
        self.assertTrue(b == div.childNodes[0])
        self.assertTrue(p == div.childNodes[1])
        
        self.assertTrue(b == div.removeChild(b))
        self.assertEqual(1, len(div.childNodes))        
        self.assertTrue(p == div.childNodes[0])
    ...
    def testAttr(self):
        ...
        self.assertEqual(html, attr.parentNode)
        self.assertFalse(attr.hasChildNodes())        
        self.assertTrue(attr.childNodes != None)
        self.assertEqual(0, attr.childNodes.length)
        self.assertFalse(attr.firstChild)
        self.assertFalse(attr.lastChild)
        self.assertFalse(attr.previousSibling)
        self.assertFalse(attr.nextSibling)
        self.assertFalse(attr.attributes)
        
        self.assertFalse(attr.hasChildNodes())        
        
        self.assertEqual(self.doc, attr.ownerDocument)

        self.assertEqual("xmlns", attr.name)        
        self.assertTrue(True, attr.specified)
        
        self.assertEqual("http://www.w3.org/1999/xhtml", attr.value)
        
        attr.value = "test"
        
        self.assertEqual("test", attr.value)
        self.assertEqual("test", html.getAttribute("xmlns"))
    ...
    def testNamedNodeMap(self):
        ...
        self.assertEqual(2, attrs.length)
        
        attr = attrs.getNamedItem("onload")

        self.assertTrue(attr)        
        self.assertEqual("onload", attr.name)
        self.assertEqual("load()", attr.value)
        
        attr = attrs.getNamedItem("onunload")
        
        self.assertTrue(attr)        
        self.assertEqual("onunload", attr.name)
        self.assertEqual("unload()", attr.value)
        
        self.assertFalse(attrs.getNamedItem("nonexists"))
        
        self.assertFalse(attrs.item(-1))
        self.assertFalse(attrs.item(attrs.length))
        
        for i in range(attrs.length):
            self.assertTrue(attrs.item(i))
        ...
        # self.assertFalse(attrs.setNamedItem(attr))    # TODO
        self.assertTrue(attrs.setNamedItem(attr)) 

        self.assertEqual("world", attrs.getNamedItem("hello").value)
        
        attr.value = "flier"
        
        self.assertEqual("flier", attrs.getNamedItem("hello").value)
        
        attrs.getNamedItem("hello").value = "world"
        
        self.assertEqual("world", attr.value)
        
        old = attrs.setNamedItem(self.doc.createAttribute("hello"))
        
        self.assertTrue(old)
        self.assertEqual(old.name, attr.name)
        self.assertEqual(old.value, attr.value)
        
        self.assertNotEqual(old, attr)
        
        self.assertEqual(attr, attrs.getNamedItem("hello"))
        
        attrs.getNamedItem("hello").value = "flier"
        
        self.assertEqual("flier", attrs.getNamedItem("hello").value)
        self.assertEqual("flier", attr.value)
        self.assertEqual("world", old.value)
        self.assertFalse(old.parent)
...
class HTMLDocumentTest(unittest.TestCase):
    ...
    def testHTMLElement(self):
        ...
        self.assertEqual('hello', p.id)   # TODO, kind of same err self.body.body
        
        p.id = 'test'
        
        self.assertEqual(p, self.doc.getElementById('test'))
        
        forms = self.doc.getElementsByName('first')
        
        self.assertEqual(1, len(forms))

        self.assertEqual('<p id="test">Hello World!</p>' +
                          '<form name="first"></form>' +
                          '<form name="second"></form>' +
                          '<a href="#">link</a>' +
                          '<a name="#">anchor</a>',
                          self.doc.getElementsByTagName('body')[0].innerHTML)
        self.assertEqual("Hello World!", p.innerHTML)
        self.assertEqual("", self.doc.getElementsByTagName('form')[0].innerHTML)

        self.assertEqual(None, self.doc.getElementById('inner'))

        self.doc.getElementsByTagName('form')[0].innerHTML = "<div id='inner'/>"

        self.assertEqual(u'DIV', self.doc.getElementById('inner').tagName)

    def testDocument(self):
        self.assertEqual("this is a test", self.doc.title)

        self.doc.title = "another title"

        self.assertEqual("another title", self.doc.title)
        ...
        self.assertEqual("another title", doc.title)        
        
        self.assertEqual(self.doc.getElementsByTagName('body')[0], self.doc.body) 
        
        forms = self.doc.forms
        
        self.assertTrue(forms != None)
        self.assertEqual(2, len(forms))
        
        self.assertTrue(isinstance(forms[0], HTMLFormElement))
        self.assertEqual("first", forms[0].name)
        self.assertEqual("second", forms[1].name)

        self.assertEqual(1, len(self.doc.links))  # TODO
        self.assertEqual(1, len(self.doc.anchors))
    ...
    def testWrite(self):
        ...
        self.assertEqual("Hello World", doc.title)

        doc.current = doc.getElementsByTagName('title')[0].tag
        doc.write("<meta/>")

        self.assertEqual("<head><title>Hello World</title><meta/></head>", str(doc.getElementsByTagName('head')[0]))
...
class CSSStyleDeclarationTest(unittest.TestCase):
    def testParse(self):
        ...
        self.assertTrue(css)
        # self.assertEqual('width: auto; font-family: serif; border: none; background: red', css.cssText)
        self.assertEqual('width: auto; border: none; font-family: serif; background: red', css.cssText)
        self.assertEqual(4, css.length)
        
        self.assertEqual('auto', css.getPropertyValue('width'))
        self.assertEqual('', css.getPropertyValue('height'))
        
        self.assertEqual('auto', css.item(0))
        self.assertEqual('auto', css.width)
        
        css.width = 'none'
        
        self.assertEqual('none', css.getPropertyValue('width'))
        self.assertEqual('none', css.item(0))
        self.assertEqual('none', css.width)
